...
AR1 <- arima(database$Close, order = c(1,0,0))
MA1 <- arima(database$Close, order = c(0,0,1))

model_212 <- arima(database$Close, order=c(2, 1, 2))
AR2 <- arima(database$Close, order = c(2,0,0))
MA2 <- arima(database$Close, order = c(0,0,2))

model_313 <- arima(database$Close, order=c(3, 1, 3))
AR3 <- arima(database$Close, order = c(3,0,0))
MA3 <- arima(database$Close, order = c(0,0,3))
...
fit_111 <- lm(residuals_111^2 ~ database$Close)
fit_AR1 <- lm(residuals_AR1^2 ~ database$Close)
fit_MA1 <- lm(residuals_MA1^2 ~ database$Close)

fit_212 <- lm(residuals_212^2 ~ database$Close)
fit_AR2 <- lm(residuals_AR2^2 ~ database$Close)
fit_MA2 <- lm(residuals_MA2^2 ~ database$Close)

fit_313 <- lm(residuals_313^2 ~ database$Close)
fit_AR3 <- lm(residuals_AR3^2 ~ database$Close)
fit_MA3 <- lm(residuals_MA3^2 ~ database$Close)
...
Les modèles ARIMA, AR et MA ont été ajusté aux données de la série temporelle des prix de clôture. Voici un résumé des résultats :
...
La valeur du critère d'information d'Akaike (AIC) est de 92801.03, ce qui est une mesure de la qualité du modèle ajusté par rapport aux données. Plus le AIC est bas, mieux c'est.
...
forecast <- forecast(model_313, h=20)
...
Le modèle prédit que le prix de clôture restera autour de 6807.479, ce qui indique une tendance constante selon le modèle.
